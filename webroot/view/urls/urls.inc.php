...
# pagination
$TemplateData['pagination'] = array('pages' => 0, 'currentGetParameters' => array('p' => 'urls'));
...
if(isset($_GET['st'])) {
    ...
    if(Helper::validate($searchValue,'nospaceP')) {
        $Urls->setQueryOptions($queryOptions);
        if($Urls->prepareSearchValue($searchValue)) {
            $TemplateData['searchresults'] = $Urls->getUrls();
...
if(!empty($TemplateData['searchresults']['amount'])) {
    ...
    $TemplateData['pagination']['sortOptions'] = $Urls->getSortOptions();
}
